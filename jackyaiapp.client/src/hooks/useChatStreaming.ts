              ...
              if (line.startsWith('data: ')) {
                ...
                switch (data.event) {
                  ...
                  case 'node_finished':
                    ...
                    const statusEmoji = status === 'succeeded' ? '✅' : status === 'failed' ? '❌' : '⏸️';
                  ...
                  case 'workflow_finished':
                    ...
                    const workflowEmoji = workflowStatus === 'succeeded' ? '🎉' : workflowStatus === 'failed' ? '💥' : '⏸️';
                    setStreamingStatus(prev => ({
                      ...prev,
                      currentEvent: 'workflow_finished',
                      statusText: `${workflowEmoji} 工作流程已完成`,
                      workflowSteps: data.data?.total_steps || 0,
                    }));
                    workflowSteps = data.data?.total_steps || 0;
